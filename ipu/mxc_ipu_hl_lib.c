...
static unsigned int ipu_ic_out_max_width_size = 1024;
static unsigned int ipu_ic_out_max_height_size = 1024;
static unsigned int ipu_vdi_in_max_width_size = 968;
static unsigned int ipu_vdi_in_max_height_size = 1024;
static unsigned int ipu_vdi_out_max_width_size = 968;
static unsigned int ipu_vdi_out_max_height_size = 1024;
...
static void _ipu_check_arch_ic_vdi_parameters()
{
	unsigned int system_rev, arch;

	if (get_system_rev(&system_rev) < 0)
		return;  // use default parameter
	...
	if ((arch == 0x37000) || (arch == 0x51000))
	{
		...
		ipu_vdi_in_max_width_size = 720;
		ipu_vdi_in_max_height_size = 512;
		ipu_vdi_out_max_width_size = 720;
		ipu_vdi_out_max_height_size = 512;
	}
	else if(arch == 0x53000)
	{
		ipu_ic_out_max_width_size = 1024;
		ipu_ic_out_max_height_size = 1024;
		ipu_vdi_in_max_width_size = 968;
		ipu_vdi_in_max_height_size = 1024;
		ipu_vdi_out_max_width_size = 968;
		ipu_vdi_out_max_height_size = 1024;
	}
}
		...
		ipu_lib_overlay_param_t * overlay)
{
	...
	_ipu_check_arch_ic_vdi_parameters();
	...
	if ((task_mode & IC_MODE) || (task_mode == NULL_MODE)) {
		...
		if (ipu_priv_handle->ic_ow > ipu_ic_out_max_width_size)
			ipu_priv_handle->split_mode |= SPLIT_MODE_RL;
		if (ipu_priv_handle->ic_oh > ipu_ic_out_max_height_size)
	...
	{
		ipu_priv_handle->split_mode = 0;

		/* whether inpu size is too big, if so, enable split mode */
		if ((ipu_priv_handle->ic_iw >ipu_vdi_in_max_width_size) || (ipu_priv_handle->ic_ow > ipu_vdi_out_max_width_size))
			ipu_priv_handle->split_mode |= (SPLIT_MODE_RL + SPLIT_MODE_VDI);
		if ((ipu_priv_handle->ic_ih > ipu_vdi_in_max_height_size) || (ipu_priv_handle->ic_oh > ipu_vdi_out_max_height_size))
			ipu_priv_handle->split_mode |= (SPLIT_MODE_UD + SPLIT_MODE_VDI);
	}
		...
		ipu_lib_handle_t * ipu_handle)
{
	...
	if (ipu_priv_handle->split_mode) {
		...
		{
			if(ipu_priv_handle->split_mode & SPLIT_MODE_VDI)
			{
				ipu_calc_stripes_sizes(ipu_priv_handle->ic_iw,
						ipu_priv_handle->ic_ow,
						((ipu_priv_handle->ic_ow >> 1) & (~(0x7))) + 8, /* half and align in 8 pixel */
				...
				if (ipu_priv_handle->left_stripe.output_width > ipu_vdi_out_max_width_size) {
					dbg(DBG_ERR, "Split mode width overflow!\n");
					ret = IPU_STATE_SPLIT_MODE_WIDTH_OVER;
					goto done;
				}
			}
			else
			{
				ipu_calc_stripes_sizes(ipu_priv_handle->ic_iw,
						...
						&ipu_priv_handle->left_stripe,
						&ipu_priv_handle->right_stripe);
				if (ipu_priv_handle->left_stripe.output_width > ipu_ic_out_max_width_size) {
					dbg(DBG_ERR, "Split mode width overflow!\n");
					ret = IPU_STATE_SPLIT_MODE_WIDTH_OVER;
					goto done;
				}
		...
		if (ipu_priv_handle->split_mode & SPLIT_MODE_UD)
		{
			if(ipu_priv_handle->split_mode & SPLIT_MODE_VDI)
			{
				ipu_calc_stripes_sizes(ipu_priv_handle->ic_ih,
						...
						((ipu_priv_handle->ic_oh >> 1) & (~(0x7))) + 8, /* half and align in 8 pixel */
						(((unsigned long long)1) << 32), /* 32bit for fractional*/
						1, /* equal stripes */
						input->fmt,
						output->fmt,
				...
				if (ipu_priv_handle->up_stripe.output_width > ipu_vdi_out_max_height_size) {
					dbg(DBG_ERR, "Split mode height overflow!\n");
			...
			else
			{
				ipu_calc_stripes_sizes(ipu_priv_handle->ic_ih,
						...
						&ipu_priv_handle->up_stripe,
						&ipu_priv_handle->down_stripe);
				if (ipu_priv_handle->up_stripe.output_width > ipu_ic_out_max_height_size) {
					dbg(DBG_ERR, "Split mode height overflow!\n");
					ret = IPU_STATE_SPLIT_MODE_HEIGHT_OVER;
					goto done;
				}
	...
	void * output_cb_arg)
{
	...
	if (ipu_priv_handle->enabled == 0) {
		...
		if (ipu_priv_handle->mode & OP_STREAM_MODE) {
			if ((ipu_priv_handle->mode & TASK_VDI_VF_MODE) && (ipu_priv_handle->output_fr_cnt == 1) && (!ipu_priv_handle->split_mode)) {
				if (ipu_priv_handle->motion_sel != HIGH_MOTION)
		...
		} else if ((ipu_priv_handle->mode & TASK_VDI_VF_MODE) && (ipu_priv_handle->motion_sel != HIGH_MOTION) && (!ipu_priv_handle->split_mode)) {
			if (ipu_priv_handle->mode & OP_STREAM_MODE) {
	...
	if ((ipu_priv_handle->mode & TASK_VDI_VF_MODE) && (ipu_priv_handle->motion_sel != HIGH_MOTION) && (!ipu_priv_handle->split_mode)) {
		if (ipu_priv_handle->mode & OP_STREAM_MODE)
